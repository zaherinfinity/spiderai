...
# Lazy processor (optional ML) — avoids import crash on serverless
try:
    from utils.processor import process_video, check_ffmpeg, executor
except Exception as _proc_err:
    print("[warn] processor unavailable:", _proc_err)
    process_video = None
    def check_ffmpeg():
        return False
    executor = None
...
app.secret_key = Config.SECRET_KEY or "vercel-dev-secret-change-me"
...
def upload():
    ...
    def run_job():
        try:
            if process_video is None:
                raise Exception("Video processing not available on this host (install requirements-full on a VPS).")
    ...
    if executor is None:
        flash("Video processing is not available on this server. Use a VPS with full requirements.", "error")
        return redirect(url_for("dashboard"))
